...
# ---------- TP4 : Bellman-Ford & RLF ----------

from tp4_algo import bellman_ford_algo, rlf_coloring
...
@app.route("/tp4/bellman", methods=["POST"])
def tp4_bellman():
    ...
    result = bellman_ford_algo(nodes, edges, source)
    ...
    image = graphe_to_base64(G, title="Bellman-Ford")
...
@app.route("/tp4/rlf", methods=["POST"])
def tp4_rlf():
    data = request.get_json()
    nodes = data.get("nodes", [])
    edges = data.get("edges", [])

    # Supprimer le poids si présent
    edges_clean = [[u, v] for u, v, *rest in edges]

    from tp4_algo import rlf_coloring
    result = rlf_coloring(nodes, edges_clean)
    return jsonify(result)


@app.route("/tp4/rlf-draw", methods=["POST"])
def tp4_rlf_draw():
    import matplotlib.pyplot as plt
    import io, base64
    import networkx as nx
